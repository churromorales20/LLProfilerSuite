<template>
  <a 
    ...
    class="relative image-container w-full md:h-56 sm:h-56 h-28 lg:h-20  ll-rounded overflow-hidden"
  >
    <img :src="imageUrl" alt="imagen" class="absolute" :class="{ 'zoomed': isZoomed }">
    <div class="md:h-56 sm:h-56 h-28 lg:h-20 bg-black/75 show-more-images-btn" v-if="isShowMoreButton">
      + {{ remaining }}
    </div>
  </a>
</template>
