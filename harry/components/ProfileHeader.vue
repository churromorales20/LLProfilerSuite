<template>
    ...
    <div class="bg-cover bg-center h-48 flex items-end rounded-b-lg"
      ...
      <div class="relative w-full lg:ml-6 lg:block flex justify-center">
        <div class="z-10 absolute bottom-0 transform translate-y-1/2">
          <UAvatar 
            :size="viewport.isLessThan('lg') ? '4xl' : '3xl'" 
  ...
  <div class="mt-24 mb-10 lg:mt-0 lg:mb-0 h-20 lg:pl-48 lg:pr-6 items-center lg:flex-row flex flex-col items-center lg:justify-between">
    <div>
      <h2 class="md:text-2xl text-xl font-semibold text-white">{{ fullName }}</h2>
      <p class="lg:mb-0 lg:text-left md:text-lg text-base text-center mb-2 ll-uname-text" v-if="!isEmptyString(profile.nickname)">{{ profile.nickname }}</p>
    </div>
    <div>
      <UButton 
        v-if="wallActive"
